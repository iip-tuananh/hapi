...
class FrontController extends Controller
{
    ...
    {
        $data['banners'] = Banner::with(['image'])->where('position', 1)->limit(1)->get();
    ...
    public function showProductDetail($slug)
    {
        ...
        // sản phẩm tương tự
        $productsRelated = $product->with([
            'product_rates' => function ($q) {
                $q->where('status', 2);
            }
        ])
        ->where('status', 1)->whereNotIn('id', [$product->id])->orderBy('created_at', 'desc')->get();
        ...
        $reviews = Review::query()->with(['image'])->orderBy('id', 'desc')->get();
        $about_us = AboutUs::query()->first();

        return view('site.products.product_detail', compact('categories', 'product', 'productsRelated', 'category', 'questions', 'reviews', 'about_us'));
    ...
    public function aboutUs()
    {
        ...
        return view('site.about_us', compact('policies', 'title', 'description', 'about_us', 'partners'));
    }
    ...
    {
        $questions = Question::query()->get();
        return view('site.faqs', compact('questions'));
    }
}
